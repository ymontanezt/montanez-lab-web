...
import { Mail, Phone, MapPin, Clock, ExternalLink, Microscope } from 'lucide-react'
import { env } from '@/lib/config/env'

// Iconos de redes sociales en color teal Montañez Lab
const FacebookIcon = () => (
  <svg className="h-5 w-5 text-teal-600 dark:text-teal-400" viewBox="0 0 24 24" fill="currentColor">
...
export function Footer({ className, variant = 'default' }: FooterProps) {
  ...
  const variants = {
    default: 'bg-gradient-to-br from-gray-50 to-gray-100 dark:from-black dark:to-gray-900',
  ...
  return (
    <footer className={cn('py-6 md:py-8 lg:py-10', variants[variant], className)}>
      <div className="container mx-auto px-4">
        {/* Header del Footer - Compacto y Elegante */}
        <div className="mb-8 text-center">
          <div className="mb-4 inline-flex items-center gap-3">
            <div
              className={`flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br ${colorTokens.gradient.brand.primary} shadow-lg dark:from-teal-500 dark:to-teal-600 dark:shadow-teal-500/30`}
            >
              <span className="text-2xl drop-shadow-sm dark:drop-shadow-md">🦷</span>
            </div>
            <div className="text-left">
              <h3 className="bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-xl font-bold text-transparent md:text-2xl dark:from-white dark:to-gray-200">
                Montañez Lab
              </h3>
              <p
          ...
          <p
            className={`mx-auto max-w-xl text-sm leading-relaxed ${colorTokens.text.muted} md:text-base dark:${colorTokens.text.tertiary}`}
          >
            Especialistas en prótesis dentales de alta calidad con tecnología avanzada y materiales
            premium para restaurar tu sonrisa perfecta.
          </p>
        </div>
            ...
            <div className="flex justify-center gap-2 md:justify-start">
              {socialLinks.map((social, index) => (
                <Link
                  ...
                  className={`group rounded-lg border ${colorTokens.border.brand.light} ${colorTokens.background.brand.light} p-2 shadow-sm transition-all duration-200 hover:scale-110 ${colorTokens.hover.background.brand.light} hover:shadow-md dark:${colorTokens.border.brand.primary} dark:${colorTokens.background.brand.muted} dark:${colorTokens.hover.background.brand.light}`}
                >
                  <social.icon />
                </Link>
              ))}
        ...
        >
          ...
          >
            <div className="flex flex-col items-center gap-1 sm:flex-row sm:gap-2">
              ...
              <span className="hidden sm:inline">Laboratorio Dental de Excelencia</span>
            </div>
